...
the array (it felt similar to the deq assignment with storing the prev and next). It is important to note that the minimum l would be 3, pointers are 8 bytes in C on 64 bit systems so having data that would be smaller than 8 bytes would probably lead to weird overlaps with the pointer memory locations.
